...
#define CROUCH_OFFSET 8
  ...
  // obj->xpos, obj->ypos, obj->collision_state, dx ,dy, jmp);
}
...
void anim_chase_graveyard(DisplayObject *obj) {
  ...
  case STATE_MOVING_RIGHT:
    dx = 2;
    if (obj->xpos == 72 || obj->xpos == 118 || obj->xpos == 150) {
      obj->state = STATE_HOPPING_RIGHT;
      obj->aux = 0;
...
void anim_ghost(DisplayObject *obj) {
  ...
  if (obj->aux == 10) {
    ...
    if (is_colliding(obj)) {
      if (is_colliding_x(obj) && sys_rand() > 128) {
        dy *= -1;
      }
      if (is_colliding_y(obj) && sys_rand() > 128) {
        dx *= -1;
      }
    }
  }
