 ...
 * Add dragon flame
 */
void add_tob_bullet(uint8_t xpos, uint8_t ypos, uint8_t tileidx,
                    uint8_t anim_id, uint8_t state, uint8_t dir, uint8_t speed,
                    DisplayObject *parent) {
  uint8_t idx;
...
void load_room(uint8_t room, bool reload) {
  ...
  map_inflate(map_map_segment_dict[room], map_map_segment[room],
              scr_tile_buffer, 192, 32);
  ...
  for (dpo = display_object, i = 0; type != 255; i++, dpo++) {
    ...
    if (type == ACTIONITEM) {
      ...
      if (action_item_type == TYPE_SCROLL) {
        ...
        if (game_state.cross_cnt == 12 || game_state.final_fight) {
      ...
      if (id == 0) {
        ...
        add_dpo = true;
                 ...
                 (game_state.cross_cnt == 12 || game_state.final_fight)) {
        /* satan door */
        add_tileobject(dpo, tob_ct, TILE_DOOR);
        dpo->visible = false;
      ...
      if (map_object->object.movable.type == TYPE_TEMPLAR) {
        add_sprite(dpo, spr_ct, PATRN_TEMPLAR);
        if (room == ROOM_FOREST || room == ROOM_GRAVEYARD) {
          if (room == ROOM_FOREST)
