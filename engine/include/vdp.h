...
extern void vdp_poke_di(uint16_t address, uint8_t value) __nonbanked;
...
extern void vdp_copy_to_vram_di(uint8_t * buffer, uint16_t vaddress, uint16_t length) __nonbanked;
extern void vdp_copy_from_vram(uint16_t vaddress, uint8_t * buffer, uint16_t length);
extern void vdp_set_hw_sprite(struct vdp_hw_sprite *spr, char spi) __nonbanked;
extern void vdp_set_hw_sprite_di(uint8_t * spr, uint16_t spi) __nonbanked;
extern void vdp_init_hw_sprites(char spritesize, char zoom);
extern void vdp_fastcopy_nametable(uint8_t * buffer) __nonbanked;
extern void vdp_fastcopy_nametable_di(uint8_t * buffer) __nonbanked;
extern void vdp_fastcopy16(uint8_t * src_ram, uint16_t dst_vram) __nonbanked;
